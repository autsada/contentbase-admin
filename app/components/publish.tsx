...
import {
  useGetProfileContractAddressFromPublishContract,
  useUpdateProfileContractAddressForPublishContract,
} from "~/hooks/publishContract"

export function Publish() {
  const [profileAddress, setProfileAddress] = React.useState("")
  const { refetch } = useGetProfileContractAddressFromPublishContract()
  const { write, isLoading, isSuccess, error, isError } =
    useUpdateProfileContractAddressForPublishContract(profileAddress)
  ...
  //   console.log("profile address/publish -->", data)
  return (
    <div className="mb-10">
